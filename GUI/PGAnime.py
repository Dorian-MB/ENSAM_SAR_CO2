...
class PGAnime:
    ...
    def _init(self):
        ...
        self.surfaces_pos = {
            "ui": (0, 0),
            "anime": (0, self.ui_height),
            "kpis": (0, self.anim_height + self.ui_height),
        }
        self._init_surfaces()
    ...
    def _set_random_port_positions(self):
        ...
        off_set_x, off_set_y = x // 10, y // 2
        random_pos = lambda: (
            random.randint(off_set_x, self.WIDTH - off_set_x),
            random.randint(off_set_y, self.HEIGHT - off_set_y),
        )
    ...
    def draw_ship(self, ship):
        ...
        if ship.state == shipState.NAVIGATING:
            ...
            try:  # Block try, pour pouvoir utiliser l'ancienne simulation : debug
                ...
                all_ship_order = dict(
                    sorted(
                        all_ship_order[ship.destination.name].items(),
                        key=lambda x: x[1],
                    )
                )
            ...
            bar_pos = (
                port_pos[0] - self.PORT_SIZE // 2,
                1 + port_pos[1] + self.PORT_SIZE // 2 + offset,
            )
            pygame.draw.rect(
                self.anim_surf,
                self.cfg.BLACK,
            ...
            pygame.draw.rect(
                self.anim_surf,
                ship.color,
                (*bar_pos, int(bar_width * progress), bar_height),
                border_radius=10,
            )
    ...
    def _draw_vertical_progress_bar(self, port, color, offset_x=4, border_radius=10, pos=None):
        ...
        pygame.draw.rect(
            self.anim_surf,
            color,
            (x, y + bar_height * (1 - progress), bar_width, bar_height * progress),
            border_radius=border_radius,
        )
    ...
    def handle_pygame_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            new_knob_x = max(
                self.slider_rect.x,
                min(
                    mouse_x - self.knob_rect.width / 2,
                    self.slider_rect.x + self.slider_rect.width - self.knob_rect.width,
                ),
            )
