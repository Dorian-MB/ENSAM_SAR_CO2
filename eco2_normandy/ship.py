...
class Ship(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.immobilization_cost_per_period = (
            immobilization_cost_per_hour * num_period_per_hours
        )
    ...
    def _save_state(self):
        ...
        self.history.append(
            {
                k: getattr(self, k) if k != "destination" else getattr(self, k).name
    ...
    def _pick_new_destination(self):
        ...
        if isinstance(self.destination, Storage):
            self.distance_to_go = get_distance(
                self.destination, self.factory, self.distances
            )
    ...
    def load_unload(self):
        ...
        match self.destination:
            case Storage():
                if self.capacity == 0:
                    return True
                transferred_amount = yield self.env.process(
                    self.destination.pump(self.capacity)
                )
            ...
            case Factory():
                ...
                transferred_amount = yield self.env.process(
                    self.destination.pump(free_cap)
                )
                if transferred_amount:
    ...
    def run(self):
        ...
        if self.state == shipState.INIT:
            ...
            self.destination = get_destination_from_name(
                self.init["destination"], self.factory, self.storages
            )
        ...
        while True:
            ...
            if self.state == shipState.NAVIGATING:
                ...
                if transfer_finished and not wait_for_bad_weather_before_leaving_port(
                    weathers, self.allowed_speeds
                ):
