...
class LiveKpisGraphsGenerator:
    ...
    def _init_graphs(self, figsize=None):
        ...
        def update_violin():
            ...
            parts_new = ax.violinplot([data], widths=0.8,
                                      showmeans=False, showmedians=True, showextrema=True, quantiles=[0.25, 0.75])
            for key, artists in parts_new.items():
                for art in np.atleast_1d(artists):
                    art.set_alpha(0.5)
                    art.set_color('red')
            box_new = ax.boxplot([data], widths=0.1, whis=1.5,
                                 patch_artist=True,
            ...
            for key, artists in parts_new.items():
                for art in np.atleast_1d(artists):
                    ax.draw_artist(art)
            for artist in box_new['boxes'] + box_new['medians'] + box_new['whiskers'] + box_new['caps'] + box_new['fliers']:
    ...
    def _init_plot_factory_capacity_evolution(self, figsize=None, init=True):
        ...
        line = ax.plot(x, y, marker=".", linestyle="-", linewidth=1, c="salmon", label="capacity") # Pourquoi ax.plot est dans une liste a 1 element ?
    ...
    def _get_data_storage_capacity_comparison(self, init=False):
        ...
        if init : 
            equal, not_equal = 0, 0
        return {
            'capacity_max == capacity': equal,
            'capacity_max > capacity': not_equal
        }
    ...
    def _init_plot_storage_capacity_comparison(self, figsize=None, init=True):
        ...
        for label, val in data.items():
            annot = ax.annotate(str(val), xy=(label, val),
