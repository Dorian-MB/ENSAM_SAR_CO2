...
class NSGA3HistoryAnalyzer:
    ...
    def plot_algorithms_comparison(results_dict, figsize=(15, 5)):
        ...
        for idx, (algo_name, res) in enumerate(results_dict.items()):
            ...
            # Hypervolume
            axes[0].plot(metrics['generation'], metrics['hypervolume'], 
                        label=algo_name, color=color, linewidth=2)
            
            # Nombre de solutions
            axes[1].plot(metrics['generation'], metrics['n_solutions'], 
                        label=algo_name, color=color, linewidth=2)
            
            # Spread
            axes[2].plot(metrics['generation'], metrics['spread'], 
                        label=algo_name, color=color, linewidth=2)
...
Périodes détectées:"""
    ...
    def visualize_evolution(self, generations_to_show=None, figsize=(15, 10)):
        ...
        for idx, gen_idx in enumerate(generations_to_show):
            ...
            if algo.opt is not None and len(algo.opt) > 0:
                ...
                ax.text(0.5, 0.5, f'Génération {gen_idx + 1}\n(Pas de solutions)', 
    ...
    def create_evolution_animation(self, filename='evolution.gif', fps=2):
        ...
        def update(frame):
            ...
            if algo.opt is not None and len(algo.opt) > 0:
                ...
                ax1.text(0.5, 0.5, 'Pas de solutions', transform=ax1.transAxes, 
                        ha='center', va='center')
                ax2.text(0.5, 0.5, 'Pas de solutions', transform=ax2.transAxes, 
                        ha='center', va='center')
    ...
    def _plot_4d_projection(self, F, ax, title):
        ...
        if len(F) > 2:
            ...
            scatter = ax.scatter(F_2d[:, 0], F_2d[:, 1], 
                               c=np.arange(len(F)), cmap='viridis', 
                               s=50, alpha=0.7)
